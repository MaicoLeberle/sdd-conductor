...
1. **Setup**
    ...
    - Read `.sdd-conductor/project.md` to get the current project specification. Check it satisfies
    `schemas/project.json`.
    - Check that `.sdd-conductor/project_snapshot.md` exists. If it does not, inform the user that
    ...
    and stop until the user chooses:
        ...
        Proceed as follows:
            ...
            `.sdd-conductor/old_versions/` and incrementing by one (e.g. `v1`, `v2`, …).
            2. Create `.sdd-conductor/old_versions/<version>/`.
            3. Copy `.sdd-conductor/tasks/current_state.md` into
            `.sdd-conductor/old_versions/<version>/current_state.md`.
            4. Copy `.sdd-conductor/tasks/pending_tasks.md` into
            `.sdd-conductor/old_versions/<version>/pending_tasks.md`.
            5. Copy every `.sdd-conductor/tasks/completed_tasks/completed_task_*.md` file into
            `.sdd-conductor/old_versions/<version>/`, preserving each file's original name. Then
            delete every `.sdd-conductor/tasks/completed_tasks/completed_task_*.md`.
            6. Copy `.sdd-conductor/architecture.md` into
            `.sdd-conductor/old_versions/<version>/architecture.md`.
            7. Copy `.sdd-conductor/modules.md` into
            `.sdd-conductor/old_versions/<version>/modules.md`.
            8. Copy `.sdd-conductor/project_snapshot.md` into
            `.sdd-conductor/old_versions/<version>/project.md`. This preserves the exact
            specification that governed the now-archived development run.
            9. Reset the STATUS section of `.sdd-conductor/tasks/current_state.md` to `NOT BLOCKED`.
            10. Re-derive the DESCRIPTION section of `.sdd-conductor/tasks/current_state.md` to
            reflect the current state of the codebase relative to the new specification: summarise
            what has already been built that remains relevant to the new `.sdd-conductor/project.md`,
    ...
    - Read `.sdd-conductor/modules.md` to get the current project module boundaries. 
    ...
    a "QA checks" section.
    - Read `.sdd-conductor/tasks/current_state.md` to get the current state of the target project
    development process. Check it satisfies `schemas/current_state.json`.
    - Read `.sdd-conductor/tasks/pending_tasks.md` to get the full ordered task list. The first entry
    ...
    - Analyze whether `.sdd-conductor/tasks/current_state.md` and
    `.sdd-conductor/tasks/pending_tasks.md` are consistent with each other, by reading the
    specifications in `.sdd-conductor/project.md`.
...
completed and correct. Follow these instructions:
    ...
    `.sdd-conductor/tasks/pending_tasks.md` and `.sdd-conductor/project.md` are in context.
    - **Code generation in target project**
        - If this is the first iteration of the cycle, implement the changes read from the first
        entry of `.sdd-conductor/tasks/pending_tasks.md`. Do not forget to add unit tests to the test
        ...
        `.sdd-conductor/tasks/pending_tasks.md`.
    - **Checks**
        ...
        `.sdd-conductor/tasks/pending_tasks.md` (instead of any
            ...
            at a conceptual level.
                - If the solution is conceptually suitable for the task:
                    - Follow the instructions in the "QA checks" of the language-specific files.
                        ...
                        `.sdd-conductor/tasks/current_state.md` to `BLOCKED`), inform the user about
...
4. **Post-task updates**
    ...
    absolutely necessary, rewrite `.sdd-conductor/architecture.md` as a compressed, accurate
    ...
    absolutely necessary, rewrite `.sdd-conductor/modules.md` as a compressed, accurate description of
    ...
    - Take `.sdd-conductor/tasks/current_state.md` and the first entry of
    `.sdd-conductor/tasks/pending_tasks.md`, and compute the new current state of the project. This
    ...
    `.sdd-conductor/tasks/current_state.md` with the newly-computed state, and set its STATUS section
    to `NOT BLOCKED`.
    - Create a new `.sdd-conductor/tasks/completed_tasks/completed_task_n.md`, where `n` is defined as
    the next sequential number (starting from 1). Copy the content of the first entry of
    `.sdd-conductor/tasks/pending_tasks.md` (all content up to but not including the first `---`
    ...
    `.sdd-conductor/tasks/pending_tasks.md`. If the file still has task entries, the new first entry
    is the next task. If the file has no task entries remaining, the project is complete: inform the
    user, change the STATUS section in `.sdd-conductor/tasks/current_state.md` to `COMPLETE` and
    ...
    - **Git**
        ...
        `.sdd-conductor/tasks/pending_tasks.md` before the post-task update above). Changes made for
        ...
        following block to `.sdd-conductor/tasks/completed_tasks/completed_task_N.md` (where N is the
