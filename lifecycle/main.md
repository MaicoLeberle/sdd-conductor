...
| Stage | Meaning |
|---|---|
| `uninitialized` | `.project-sdd/` does not yet exist |
| `bootstrapped` | `.project-sdd/` exists; tasks have not yet been derived |
...
be `in_progress` but the STATUS section of `.project-sdd/tasks/current_state.md` reads `BLOCKED`.

`corrupted` is an error state, not a lifecycle stage. It can occur at any point after bootstrap if
required files are missing from `.project-sdd/`. It is detected during stage computation and causes
...
1. **Uninitialized**: if `.project-sdd/` does not exist, then stage is `uninitialized`.
2. **Corrupted**: if `.project-sdd/` does exist but any of `project.md`, `architecture.md`,
`modules.md`, `tasks/current_state.md`, or `tasks/pending_tasks.md` is missing, then the stage is
`corrupted`.
3. **Bootstrapped**: if `.project-sdd/tasks/pending_tasks.md` has no task entries (i.e., contains
no `## Title` section) AND `.project-sdd/tasks/completed_tasks/completed_task_1.md` does not exist,
then stage is `bootstrapped`.
4. **Initialized plan**: if `.project-sdd/tasks/pending_tasks.md` has task entries AND
`.project-sdd/tasks/completed_tasks/completed_task_1.md` does not exist, then stage is
`plan_is_ready`.
5. **Executing plan**: if `.project-sdd/tasks/pending_tasks.md` has task entries AND
`.project-sdd/tasks/completed_tasks/completed_task_1.md` exists:
   - Read `.project-sdd/tasks/current_state.md`. If its STATUS section reads `BLOCKED`, stage is
   `blocked`. Otherwise (STATUS is `NOT BLOCKED`), stage is `in_progress`.
6. **Complete**: if `.project-sdd/tasks/pending_tasks.md` has no task entries (i.e., contains no
`## Title` section) AND `.project-sdd/tasks/completed_tasks/completed_task_1.md` exists, then stage
is `complete`.

**Note on spec changes**: a spec pivot (Option B in `task_execution.md`) re-derives tasks and
deletes all `completed_task_*.md` files from `.project-sdd/tasks/completed_tasks/`. After a pivot,
...
preconditions. Inform the user which required files are missing from `.project-sdd/` and stop
...
- `plan_is_ready`, `in_progress`, or `blocked`: load `.project-sdd/tasks/current_state.md` and
  `.project-sdd/tasks/pending_tasks.md` (the first entry is the next task to execute).
- `complete`: load `.project-sdd/tasks/current_state.md` for a summary of what was built.
