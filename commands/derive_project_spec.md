<mark>THIS FILE CONTAINS THE INSTRUCTIONS FOR THE derive_project_spec COMMAND.</mark>
...
`.project-sdd/project.md`, and summarises what has already been built into
`.project-sdd/tasks/current_state.md`. It then pauses for user review before any task derivation.
...
4. **Derive architectural context**. Based on the codebase exploration above, populate:
   - `.project-sdd/architecture.md`: describe the high-level architecture already in place (layers,
     subsystems, key design patterns in use, and how data flows between them).
   - `.project-sdd/modules.md`: describe the module boundaries present in the codebase (modules,
...
5. **Summarise the current development state**. Based on the exploration above, write a concise
   ...
   `.project-sdd/tasks/current_state.md`. The STATUS section must be set to `NOT BLOCKED`.
   The file must satisfy `schemas/current_state.json`.

6. **Present and pause**. Show the user the generated `.project-sdd/project.md`,
   `.project-sdd/architecture.md`, and `.project-sdd/modules.md`, with a brief explanation of the
